...
class BookAsBookmark extends StatefulWidget {
  final String memberId;
  BookAsBookmark(this.memberId);
  @override
  _BookAsBookmarkState createState() => _BookAsBookmarkState();
}

class _BookAsBookmarkState extends State<BookAsBookmark> {
  String bookUrl;
  List<String> searchs = [];
  List<String> bookIds = [];
  ...
  Future<void> getBookAsBookmark() async {
    try {
      final url = serverName + '/book/view_as_bookmark';
      ...
      if (data != '{error}') {
        ...
        print('book id:' + bookIds.toString());
  ...
  Future<void> showBookmork(String bookId, int index) async {
    try {
      ...
      print('isBookmark:' + isBookmarks.toString());
    } catch (e) {
      print('show like error:' + e.toString());
    }
  }
