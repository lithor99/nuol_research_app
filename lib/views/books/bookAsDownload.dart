...
class BookAsDownload extends StatefulWidget {
  final String memberId;
  BookAsDownload(this.memberId);
  @override
  _BookAsDownloadState createState() => _BookAsDownloadState();
}

class _BookAsDownloadState extends State<BookAsDownload> {
  String bookUrl;
  List<String> searchs = [];
  List<String> bookIds = [];
  ...
  Future<void> getBookAsDownload() async {
    try {
      // preferences = await SharedPreferences.getInstance();
      final url = 'http://192.168.43.191:9000/book/view_as_download';
      Map body = {'top': preferences.getString('topDownload')};
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: MyAppBar(
        title: 'ບົດທີ່ມີຍອດດາວໂຫຼດ' +
            preferences.getString('topDownload') +
            'ອັນດັບທຳອິດ',
        fontSize: 20,
      ).myAppBar(),
      body: Container(
