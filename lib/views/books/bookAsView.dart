...
class BookAsView extends StatefulWidget {
  final String memberId;
  BookAsView(this.memberId);
  @override
  _BookAsViewState createState() => _BookAsViewState();
}

class _BookAsViewState extends State<BookAsView> {
  String bookUrl;
  List<String> searchs = [];
  List<String> bookIds = [];
  ...
  Future<void> getBookAsView() async {
    try {
      final url = serverName + '/book/view_as_view';
      Map body = {'top': preferences.getString('topView')};
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: MyAppBar(
        title: 'ບົດທີ່ມີຍອດການອ່ານ' +
            preferences.getString('topView') +
            'ອັນດັບທຳອິດ',
        fontSize: 20,
      ).myAppBar(),
      body: Container(
                            ...
                            if (status.isGranted) {
                              MyAlertDialog(
                                ...
                                onOkay: () async {
                                  ...
                                  //   bookNames[i],
                                  // );
                                  await DownloadBookFile.downloadFile(
                                    bookIds[i],
                                    bookUrl,
                                    bookNames[i],
                                  );
